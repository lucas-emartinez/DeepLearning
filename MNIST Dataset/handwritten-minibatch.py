...
def adjust_weights(x):
    ...
    delta_matrix = np.zeros((len(hidden_layer_error[:, 0]), len(x[:, 0])))

    for i in range(BATCH_SIZE):
        delta_matrix += np.outer(hidden_layer_error[:, i], x[:, i]) * LEARNING_RATE

    delta_matrix /= BATCH_SIZE
    hidden_layer_w -= delta_matrix

    hidden_output_array = np.concatenate((np.ones((1, BATCH_SIZE)), hidden_layer_y))
    delta_matrix = np.zeros((len(output_layer_error[:, 0]), len(hidden_output_array[:, 0])))

    for i in range(BATCH_SIZE):
        delta_matrix += np.outer(output_layer_error[:, i], hidden_output_array[:, i]) * LEARNING_RATE
...
for i in range(EPOCHS):  # Train EPOCHS iterations
    ...
    for j in index_list:  # Train on all examples
        ...
        j *= BATCH_SIZE  # Para obtener el Batch de entrenamiento específico
        ...
        for k in range(BATCH_SIZE):
            if output_layer_y[:, k].argmax() == y[:, k].argmax():
    ...
    for j in range(0, (len(x_test)-BATCH_SIZE), BATCH_SIZE):  # Evaluate network
