...
def backward_pass(y_truth):
    ...
    hidden_weighted_error = np.matmul(output_layer_w[:, 1:].T, output_layer_error)
    hidden_layer_error = (hidden_tanh_prime * hidden_weighted_error)
...
def adjust_weights(x):
    ...
    delta_matrix = np.outer(hidden_layer_error, x) * LEARNING_RATE
    hidden_layer_w -= delta_matrix
    hidden_output_array = np.concatenate((np.array([1.0]), hidden_layer_y))
    delta_matrix = np.outer(output_layer_error, hidden_output_array) * LEARNING_RATE
